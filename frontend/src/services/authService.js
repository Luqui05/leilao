...
const authService = {
  login: async (email, senha) => {
    try {
      ...
      if (!err.response) {
        throw new Error(`Network Error: não foi possível conectar ao servidor em ${api.defaults.baseURL}. Verifique se o backend está em execução e se a URL está correta.`);
      }
      // normalize error
  ...
  changePasswordWithCode: async ({ email, codigo, novaSenha }) => {
    try {
      await api.post('/auth/change-password-with-code', { email, codigo, novaSenha });
    } catch (err) {
      if (!err.response) {
        throw new Error(`Network Error: não foi possível conectar ao servidor em ${api.defaults.baseURL}. Verifique se o backend está em execução e se a URL está correta.`);
      }
